...
assert (BOARDWIDTH * BOARDHEIGHT) % 2 == 0, 'Must be an even number'
...
def splitInfoGroupsOf(groupSize, theList):
	"""Splits the list into a list of lists
	"""
	result = []
...
def startGameAnimation(board):
	...
	boxGroups = splitInfoGroupsOf(8, boxes)
...
def hasWon(revealedBoxes):
	for box in revealedBoxes:
		if False in box:
			return False
	return True

def leftTopCoordsOfBox(boxx, boxy):
	left = int(boxx * (BOXSIZE + GAPSIZE) + XMARGIN)
	top = int(boxy * (BOXSIZE + GAPSIZE) + YMARGIN)
	return (left, top)
